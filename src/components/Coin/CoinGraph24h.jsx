...
const CoinGraph24h = ({data}) => {
  ...
  const CustomTootip = ({active,payload,label}) => {
      if(active && payload && payload.length){
        const price = Number(payload[0].value).toFixed(4);
      ...
      <ResponsiveContainer width='100%' height={400}>
        ...
        >
          ...
          <XAxis
            ...
            tickFormatter={(date) => format(date,'h:m a')}
            tickCount = {10}
            domain={['auto', 'auto']}
            dataKey='timestamp'
          />
